...
def check_exchange(config: Config, check_for_bad: bool = True) -> bool:
    ...
    if exchange == "oanda":
        from freqtrade.forex.config import validate_native_forex_config

        validate_native_forex_config(config, config["runmode"])
        logger.info('Native forex exchange "oanda" configuration is valid.')
        return True
