...
function config() {
    echo_block "Creating the forex user directory and config skeleton"
...
function install_forex() {
    echo_block "Installing the forex project without prompts"
    ...
    if [ "$(uname -s)" == "Darwin" ]; then
        ...
        echo "Unsupported package manager. Install Python ${SUPPORTED_PY_VERSIONS}, pip, and venv first."
        exit 1
    fi
    ...
    echo "Then open http://SERVER_IP:8090/setup in a browser."
}
...
function install() {
    ...
    echo "Run 'python -m freqtrade.forex setup' to create the forex config skeleton."
